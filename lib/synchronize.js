  ...
  function checkUpdatedUser(apiKey, email) {
    return new Promise((resolve, reject) => {
      ...
      ListUser.find({
        ...
        if (lists.length == 0) return resolve(null);
        ...
        relations.forEach(function (relation, index) {
          ...
          var upWorks = newSongs.map(function (song) {
            return {
              ...
              imageId: relation.imageId, //TODO: Quitarlo de aquí y que el demonio lo busque a la hora de subir musica
    ...
    setImage: function(email, listId, listName, songId, imageId) {
      return new Promise((resolve, reject) => {
        Promise.all([
          ...
          List.findOne(
            { listId }, 
            { songs: { $elemMatch: { songId }}}
          )
        ]).then(([listUser, listDetails]) => {

          logger.debug(JSON.stringify(listDetails));
          var gmusicId = listDetails.songs[0].gmusicId ? listDetails.songs[0].gmusicId : null;
          ...
          WorkTodo.insertMany([
            ...
            { //TRABAJO DE SUBIDA, YA CON LA NUEVA IMAGEN
              songId,
              listId,
              listName,
              gmusicId,
        ...
        //     { //TRABAJO DE BORRADO
        //       songId,
        //       gmusicId: song.gmusicId, //lo tengo que conseguir
        //       listId,
        //       listName,
        //       email,
        //       state: "del",
    ...
    retryDownload: function () {
      return new Promise((resolve, reject) => {
        WorkTodo.updateMany({ 
          ...
          logger.debug("Download works in error retried.");
    ...
    retryUpload: function () {
      return new Promise((resolve, reject) => {
        WorkTodo.updateMany({ 
          ...
          logger.debug("Upload works in error retried.");
          return resolve(result.nModified);
        });
      });
    },

    getWorkDoneList: function (parms) {
      return new Promise((resolve, reject) => {
        WorkDone.aggregate([
          { $match: 
            {$and: [
              { dateLastMovement: { $gt: parms.dateStart, $lt: parms.dateEnd}}, 
              { "email": {$regex: parms.email}}
            ]}
          },
          { $group: {
            _id: { year: { $year: "$dateLastMovement" } , month: { $month: "$dateLastMovement" }, day: { $dayOfMonth: "$dateLastMovement" }, "action": "$action"},
            count: {$sum: 1} }
          },
          { $sort: {_id: 1} }
        ], function (err, list) {
