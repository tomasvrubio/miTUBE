...
module.exports = function(){
    ...
    return {
        ...
        createList: function(apiKey, listId, nameYT) {
            ...
            return new Promise((resolve, reject) => {
                //TODO: Comprobar que la lista no existe. Si ya existe (creada por otro usuario) dar la función por finalizada OK. TODO: Comprobar qeu funciona lo introducido. DE MOMENTO NO FUNCIONA. REVISAR!!
                List.findOne({listId: listId}, function(err, list){
                    if(err){
                        console.error(err.stack);
                        return reject(new Error('No posible buscar listas.'));
                    }
