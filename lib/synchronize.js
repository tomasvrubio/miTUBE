...
module.exports = function () {
  ...
  return {
    ...
    deleteRelation: function (email, listId) {
        ...
        Promise.all([
          ...
          if (songsDelete.length) {
            generateWorkDelete(listId, songsDelete).then(returnObject => {
              ...
              //Eliminar registro de la lista para el usuario (delete)
      ...
      //Comprobar si más usuarios tienen esa lista. Si no hay nadie más borrar la lista de la BBDD
    ...
    setImage: function(email, listId, imageId) {
      ListUser.findOneAndUpdate({ 
        email,
        listId
      }, {
        ...
        if (err) logger.debug(JSON.stringify(err.stack));

        logger.debug("User " + email + " put list " + listId + " imageId to " + imageId);
