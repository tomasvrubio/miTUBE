  ...
  function assignMac (macMax) {
    if (macMax){
      ...
      if (incrementalNumber<255){
        newMacArray[5] = (incrementalNumber+1).toString(16).toUpperCase();
      }
      else{
        incrementalNumber = parseInt(newMacArray[4],16);
        newMacArray[4] = (incrementalNumber+1).toString(16).toUpperCase();
        newMacArray[5] = "00";
      }        
    } else {
      ...
      var newMacArray = [
        ...
        Math.floor(Math.random()*240).toString(16).toUpperCase(),
        "00"
      ];
    }
    ...
    createFirstUser: function (email, name, gmailUser, res, mailTransport) {
      return new Promise((resolve, reject) => {
        ...
        newUser.save(function(err) {
          ...
          // res.render("email/email_register", {layout: null, cart: cart}, function(err,html){
          //   if(err) logger.error("Problems generating email: "+JSON.stringify(err.stack));
          //   mailTransport.sendMail({
          //     from: '"mitube": '+gmailUser,
          //     to: cart.email,
