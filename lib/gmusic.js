    ...
    return {
            ...
            return new Promise((resolve, reject) => {
                ...
                child.stderr.on("data", function (data) {
                    ...
                    if (dataString.includes("Successfully uploaded") || dataString.includes("ALREADY EXISTS")){
                        response = {
                            code: 0,
                            message: dataString.match(/\(.*?\)/g)[1].match(/\(([^)]+)\)/)[1], //Return uuid
                        };
                    };
                ...
                child.on("exit", function (code) {
                    if (code==1) return reject(code);
                    return resolve(response);
                    //TODO: Devolver uuid. ¿puedo hacer de alguna manera que sólo pinte eso al salir gmupload?
        ...
        delete: function(email, password, mac, gmusicId) {
            return new Promise((resolve, reject) => {
                ...
                //Lo malo es que tengo que informar de la password del usuario con -p
