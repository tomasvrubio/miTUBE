...
module.exports = function () {
  return {
	...
	if (action == "upl") {
          ...
          var child = process.spawn("gms", ["upload", "--uploader-id", mac, "-u", email, "-vvv", "fake_song.mp3"], {shell:true});
        } else if (action == "del") {
          var child = process.spawn("gms", ["delete", "--device-id", mac, "-u", email, "-vvv", "-f", "id[fake_song]", "-y"], {shell:true});
        }
        ...
        child.stderr.on("data", function (data) {
          ...
          if (dataString.includes("NOT_SUBSCRIBED")) {
            response = {
              code: 3,
              message: "Usuario si nun método de pago asignado en Google Play. Es necesario para subir música a sus servidores (gratis).",
              url: urlAuth,
            };
          }
    ...
    upload: function (email, mac, songId, imageId) {
      return new Promise((resolve, reject) => {
        ...
        var child = process.spawn("gms", ["upload", "--uploader-id", mac, "-u", email, "./tmp/" + songId + ".mp3", "--album-art", "../public/" + imageId, "--no-use-hash", "-vvv"]);
        var response = {};
        ...
        child.stdout.on("data", function (data) {
          ...
          if (dataString.includes("Found 1 songs already exist")) {
            response = {
              code: 0,
              uuid: 0,   //Return uuid not existing
            };
          };
    ...
    delete: function (email, mac, gmusicId, songId) {
      return new Promise((resolve, reject) => {
        //Return:
        //  * reject if there is a problem
        //  * code=0 : Song deleted or no song to delete
        ...
        //To solve problems when not having gmusicID
        if (gmusicId == 0 || gmusicId == null){
          logger.error("No gmusicId, deleting with songId.");
          var child = process.spawn("gms", ["delete", "-u", email, "--device-id", mac, "-f", "composer["+songId+"]", "-vvv", "-y"]);
        } else {
          var child = process.spawn("gms", ["delete", "-u", email, "--device-id", mac, "-f", "id["+gmusicId+"]", "-vvv", "-y"]);
        }
